...
// 카테고리 타입
type CategoryType = "skincare" | "makeup" | "body" | "hair" | "health" | "food" | "etc";
...
const CATEGORIES: CategoryType[] = ["skincare", "makeup", "body", "hair", "health", "food", "etc"];
...
export default function CategoryPage() {
  ...
  const loadCampaigns = async (nextPage: number) => {
    ...
    // TODO: 실제 API 호출로 변경
    // const response = await fetch(`/api/campaigns?category=${selectedCategory}&page=${nextPage}&limit=10`);
      ...
      const newItems: CampaignItem[] = Array.from(
        ...
        (_, i) => {
          ...
          return {
            ...
            dDay: `D-${(index % 10) + 1}`,
            brand: ["아르마니 뷰티", "정샘물", "듀오버스터", "LANCOME", "DERMATORY"][
              index % 5
            ],
            product: [
  ...
  }, [selectedCategory, campaignItems.length, isLoading]);
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-50 border-b border-[#eee] bg-white">
        <div className="flex items-center px-5 py-3">
          ...
          <h1 className="text-[18px] font-semibold">카테고리</h1>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2 my-3">
          ...
          <div className="relative" ref={filterDropdownRef}>
            <button
              ...
              "
            >
              {filterLabels[selectedFilter] || selectedFilter} <span className="text-[10px]">▼</span>
            </button>
